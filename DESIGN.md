...
- Maßnahme: **Gratverfolgung auf der Distanztransformation** statt Skelett-Ablauf (`src/lib/centerline.ts`):
  ...
  | Häkchen | Die mediale Achse hat an **jeder** Ecke einen Ast, der auf der Winkelhalbierenden in die Ecke zeigt. Erkennung: dort ist die nächstgelegene Ecke nur 1/sin(α/2)·Abstand entfernt (bei 90° das 1,41-fache des Randabstands) bzw. liegt innerhalb von 0,85·dMid der Eckspitze. Solche Enden werden gekappt und danach **geradeaus bis an den Rand verlängert** – der Strich kommt mittig am Ende an, ohne Bogen oder Haken in die Ecken. Spitzen bleiben stehen (dort ist die Ecke ein Vielfaches weiter weg), und Ecken mit über 145° werfen kein Häkchen |
  | T-Zweige | Abgehende Äste an T-Verzweigungen (z. B. der obere Haken einer „1“ an den Stamm oder der Mittelzwickel der „3“) werden in einem Zug angebunden (`joinBranches`): der kürzere Ast (Spitze der 1) wird bis zum Scheitel durchlaufen und der Strich führt ohne Abheben den Stamm hinab („wie in einer Linie gezeichnet“) |
  | Löcher | Außenkonturen werden vor Innenlöchern bearbeitet; Löcher erfolgreich berechneter Strich-Regionen werden als verbraucht markiert und nicht erneut als Umriss graviert (verhindert Geisterlinien bei 0, 4, 6, 8, 9 und Buchstaben mit Zählern) |
  | Einheiten | **Nichts hängt am Pixel.** Alle Größen des Laufs – Schrittweite, Stützweite der Hesse-Matrix, Fenster für das lokale Maß, Schleifen- und Ring-Erkennung, Startpunkte, Schwellen für Reste – werden am örtlichen Randabstand d (halbe Strichbreite, gerechnet in Pixeln) gemessen. Grund: die Rasterweite richtet sich nach Baugröße, Toleranz und geschätzter Strichbreite, zwischen 0,02 und 0,2 mm. Dieselbe Form kam sonst bei 25 mm und bei 200 mm heraus, als wäre es eine andere („B“: 1/2/1/2 Züge, „1“: 2/2/2/3, Kamm: 40/41/41/41). Jetzt: „O“ über vier Größen auf 0,5…1,9 % genau, Balken und Rahmen auf 0,1 %. Die Hesse-Matrix wird zusätzlich mit ihrer Stützweite multipliziert – dann hat der Wert am Grat bei jeder Rasterweite dieselbe Bedeutung (≈ −2) |
  ...
  | Verbinden | zwei Enden werden nicht einfach mit einer Sehne verbunden, wenn ihre Richtungen auseinanderlaufen: dann wird der Schnittpunkt der beiden Schenkel dazwischengesetzt – aber nur, wenn der Umweg klein bleibt (≤ 2,8 × Lücke). Bei einem Bogen liegt der Schnitt weit draußen, dort bleibt es bei der Sehne. An einer Ecke zeigen die beiden Richtungen gegeneinander – dort gilt die Regel „kein Rückwärtsgang“ nicht, sonst bleibt die Ecke offen („M“: ein Zug mit harten Ecken bei 1,85/7,25 und 6,77/7,25). Liegt der Schnitt knapp außerhalb (sehr spitze Ecke), wird er auf der Winkelhalbierenden zurückgezogen |
  | Ringe | ein Lauf, der auf sich selbst zurückkommt, wird als Ring geschlossen. Geprüft wird gegen die *eigenen* früheren Punkte (Raster mit Stempel, verkettete Liste je Zelle), nicht nur gegen den Startpunkt: ein Lauf driftet auf gebogenem Grat um ein paar Pixel je Runde nach innen und trifft den Startpunkt deshalb nicht genau – er drehte sonst eine zweite Runde und der Weg wurde um bis zu 19 % zu lang („O“ bei 12 mm) |
