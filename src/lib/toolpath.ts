...
import { buildContours, chainSegments, hatchFill, offsetPolygon, pathLength, sliceMesh } from './geometry';
...
export function computeToolpath(om: OrientedMesh, contours: Contour[], s: Settings): Toolpath | null {
  ...
  // Werkzeugradius für Offset (Schaftfräser). Stichel „on“ → radius 0.
  const radius = s.pathSide === 'on' ? 0 : s.tool.shaftDia / 2;

  // Strategie → 2D-Pfade
  let paths: { pts: Vec2[]; closed: boolean }[] = [];
  if (s.strategy === 'contour') {
    paths = active.map((c) => {
      if (!c.closed || radius === 0) return { pts: c.pts, closed: c.closed };
      // outside = Material bleibt innen → Offset nach außen
      // inside = Tasche → Offset nach innen
      const dist = s.pathSide === 'outside' ? radius : -radius;
      const off = offsetPolygon(c.pts, dist);
      if (pathLength(off, true) < s.minLength) return { pts: c.pts, closed: true };
      return { pts: off, closed: true };
    });
    ...
    // Fläche: Konturen mit optionalem Offset + Schraffur
    const closed = active.filter((c) => c.closed).map((c) => {
      if (radius === 0) return c;
      const dist = s.pathSide === 'outside' ? radius : s.pathSide === 'inside' ? -radius : 0;
      if (dist === 0) return c;
      return { ...c, pts: offsetPolygon(c.pts, dist) };
    });
    const lines = hatchFill(closed, Math.max(s.stepOver, 0.05), 45);
    paths = [
      ...closed.map((c) => ({ pts: c.pts, closed: true })),
      ...lines.map((pts) => ({ pts, closed: false })),
    ];
  }
  if (!paths.length) return null;

  // Spiegeln (Rückseitengravur) vor Nullpunkt – um Y der Bounding-Box-Mitte
  if (s.mirrorY) {
    const midY = (om.min[1] + om.max[1]) / 2;
    paths = paths.map((p) => ({
      ...p,
      pts: p.pts.map((q) => ({ x: q.x, y: 2 * midY - q.y })),
    }));
  }
